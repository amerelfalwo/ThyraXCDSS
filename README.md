...
graph TD
    ...
    classDef router fill:#bbdefb,stroke:#1976d2,stroke-width:1px;
    ...
    %% Client Layer
    Client[Streamlit Frontend / API Client]:::client

    %% Auth Layer
    Auth{API Key Validation<br/>X-AI-Service-Key}:::api

    %% API Layer (FastAPI)
    subgraph FastAPI_Backend [FastAPI Backend]
        R1[Clinical Router]:::router
        R2[Image Router]:::router
        R3[FNAC Router]:::router
        R4[Patient Router]:::router
        R5[Agent Router]:::router
    end

    %% State & Memory
    State[In-Memory Patient State<br/>TTL Cache]:::db

    %% ML & DB Layer
    subgraph Local_ML_Models [Local ML/DB Layer]
        M1[XGBoost Classifier]:::model
        M2[ONNX US Gatekeeper]:::model
        M3[ONNX FNAC Bethesda]:::model
        VDB[(ChromaDB Vector Store)]:::db
    end

    %% External Services
    LLM[Groq Llama-3.1-8B API]:::external

    %% Connections
    Client -->|HTTP POST/GET| Auth
    Auth -->|Authenticated| FastAPI_Backend
    ...
    R5 <-->|Context Retrieval| State
    R5 <-->|Semantic Search| VDB
    R5 <-->|Prompt Generation| LLM

    %% Dependencies
    M1 -.->|Lazy Load/LRU Cache| FastAPI_Backend
    M2 -.->|Lazy Load/LRU Cache| FastAPI_Backend
    M3 -.->|Lazy Load/LRU Cache| FastAPI_Backend
```
...
| POST   | /fnac/predict                   | Yes  | Cytopathology Bethesda classification      |
...
| POST   | /agent/chat                     | Yes  | AI assistant (supports multipart uploads)   |
...
ThyraX-CDSS/
├── app/
│   ├── agent/       # Agent logic and fallback handlers
│   ├── core/        # Security, config, and audit systems
│   ├── disease/     # XGBoost classification logic
│   ├── routers/     # API endpoint definitions
│   ├── services/    # Business logic and state management
│   └── schemas/     # Pydantic data models
├── data/            # ChromaDB and audit logs
├── models/          # Optimized ONNX and Joblib models
